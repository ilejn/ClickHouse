...
{
    LOG_WARNING(log, "top of StorageMergeTree::selectPartsToMutate, {} elements in parts vector",
        getDataPartsVectorForInternalUsage().size());
    ...
    for (const auto & part : getDataPartsVectorForInternalUsage())
    {
        ...
        {
            LOG_TRACE(log, "skipping part {} as currently merging/mutating", part->name);
            continue;
        }
...
MutationCommands StorageMergeTree::getAlterMutationCommandsForPart(const DataPartPtr & part) const
{
    ...
    for (const auto & [mutation_version, entry] : current_mutations_by_version | std::views::reverse)
    {
        ...
        if (entry.affectsPartition(part->info.partition_id))
